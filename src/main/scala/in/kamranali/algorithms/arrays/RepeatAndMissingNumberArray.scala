...
class RepeatAndMissingNumberArray {
  ...
  def repeatedNumber(A: Array[Int]): Array[Int]  = {

    val length = A.length

    val s1: Long = A.sum
    val s2: Long = length*(length + 1) / 2
    val p: Long = s2 - s1

    val ss1: Long = A.map(x => x*x).sum
    val ss2: Long = length*(length + 1)*(2*length + 1) / 6
    val t = (ss2 - ss1) / p
...
//    println(miss)
    ...
    Array[Int](dup.intValue, miss.intValue)
...
object RepeatAndMissingNumberArrayest extends App {
  val sol = new RepeatAndMissingNumberArray()

  // Test 1
  var A = Array[Int](3, 1, 2, 5, 3)
  var data = sol.repeatedNumber(A)
  assert(data sameElements Array[Int](3, 4))
  ...
  // Test 2
   ...
   data = sol.repeatedNumber(A)
  assert(data sameElements Array[Int](213, 87))
