...
object MultiplyStringsHard {
  ...
  def multiplyStrings(a: String, b: String): String = {
     ...
     */
      ...
      def addTwoTailrec(remainingA: List[Int], remainingB: List[Int], carry: Int, acc: List[Int]): List[Int] = {
        ...
        else if (remainingB.isEmpty) acc.reverse ++ addTwoTailrec(List(carry), remainingA, 0, List())
        else {
