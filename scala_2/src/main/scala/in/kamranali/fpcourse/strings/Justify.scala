...
object Justify {
  ...
  /*
    Justify a Big text within a given number of characters
    - row width is constant and equal to given number of characters
    - For every row the words are evenly distributed inside that row
   */

  def justify(text: String, width: Int): String = {

    def createSpaces(n: Int): String = (1 to n).map(_ => " ").mkString("") // create n spaces

    @tailrec
    def pack(words: List[String], currentRow: List[String] = List(), currentCharCount: Int = 0, res: List[List[String]] = List()): List[List[String]] = {

      if (words.isEmpty && currentRow.isEmpty) res
      else if (words.isEmpty) res :+ currentRow
      else if (currentRow.isEmpty && words.head.length > width)  {
        // split the word into intro-duction
        ...
        pack(words, List(), 0, res :+ currentRow)
      else
        pack(words.tail, currentRow :+ words.head, currentCharCount + 1 + words.head.length, res)
    ...
    val unjustifiedRows = pack(words)

    // Justify the rows
    val justifiedRows = unjustifiedRows.init.map(justifyRow) :+ unjustifiedRows.last.mkString(" ") // Unjustify the last row
  ...
  def main(args: Array[String]): Unit = {
    println(justify("Scala is the most amazing language you will ever write any code in", 15))
